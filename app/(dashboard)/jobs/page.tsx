...
import { Archive, ArrowDown, ArrowUp, BriefcaseBusiness, CalendarDays, Check, ChevronDown, ChevronLeft, ChevronRight, CircleAlert, Copy, Eye, Filter, Grid2x2X as Grid2X2, List, MoveHorizontal as MoreHorizontal, Pause, Plus, RotateCcw, Search, SlidersHorizontal, UserRoundPlus, Users, X } from 'lucide-react';
...
function SummaryStrip() {
  return (
    <section aria-label="Jobs summary" className="flex gap-1 overflow-x-auto rounded-lg border border-border bg-surface p-1 shadow-sm md:grid md:grid-cols-5 md:gap-0 md:p-0">
      {summaryItems.map((item, index) => {
        const Icon = item.icon;
        return (
          <div key={item.label} className={cn('min-w-[176px] flex-1 px-4 py-4 md:min-w-0 md:px-5', index > 0 && 'md:border-l md:border-border')}>
            ...
            <p className={cn('mt-2 flex items-center gap-1 text-caption', item.direction === 'down' ? 'text-error-ink' : 'text-success-ink')}>
              ...
              {item.delta} <span className="text-muted">vs last 30 days</span>
            </p>
          </div>
        );
      })}
  ...
  return (
    <div className="flex min-w-max divide-x divide-border overflow-hidden rounded-md border border-border bg-surface-2">
      {job.stages.map((stage) => {
        const content = (
          <span className={cn('flex min-w-[76px] flex-col items-center gap-1 px-2 py-2 text-center transition-colors', !archived && 'hover:bg-surface-hover focus-visible:bg-surface-hover')}>
            <span className="text-body font-medium tabular text-heading">{stage.count}</span>
            <span className="max-w-[72px] truncate text-[11px] leading-4 text-muted">{stage.label}</span>
...
export default function JobsPage() {
  ...
  React.useEffect(() => { setVisibleCount(5); }, [tab, search, status, sort]);
  ...
  return (
    <div className="min-h-screen bg-background px-4 py-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mt-6 flex flex-col gap-4 border-b border-border sm:flex-row sm:items-end sm:justify-between"><div className="flex gap-6" role="tablist" aria-label="Job views"><button type="button" role="tab" aria-selected={tab === 'active'} onClick={() => setTab('active')} className={cn('border-b-2 px-1 pb-3 text-button transition-colors', tab === 'active' ? 'border-primary text-primary-ink' : 'border-transparent text-muted hover:text-heading')}>Active jobs <span className="ml-1 rounded-full bg-surface-2 px-1.5 py-0.5 text-caption">{activeJobs.length}</span></button><button type="button" role="tab" aria-selected={tab === 'archived'} onClick={() => setTab('archived')} className={cn('border-b-2 px-1 pb-3 text-button transition-colors', tab === 'archived' ? 'border-primary text-primary-ink' : 'border-transparent text-muted hover:text-heading')}>Archived jobs <span className="ml-1 rounded-full bg-surface-2 px-1.5 py-0.5 text-caption">{archivedJobs.length}</span></button></div><div className="flex items-center gap-2 pb-3"><Button variant="secondary" size="sm"><Filter className="h-4 w-4" aria-hidden="true" />Filters</Button><Button variant="secondary" size="sm"><CalendarDays className="h-4 w-4" aria-hidden="true" />Last 30 days<ChevronDown className="h-4 w-4" aria-hidden="true" /></Button></div></div>
        <div className="mt-4 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between"><div className="relative w-full lg:max-w-md"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted" aria-hidden="true" /><Input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search by title, location or department" aria-label="Search jobs" className="pl-9" /></div><div className="flex flex-wrap items-center gap-2"><Select value={status} onValueChange={setStatus}><SelectTrigger className="w-32"><SelectValue placeholder="Status" /></SelectTrigger><SelectContent><SelectItem value="all">All status</SelectItem><SelectItem value="Active">Active</SelectItem><SelectItem value="Paused">Paused</SelectItem><SelectItem value="Expired">Expired</SelectItem></SelectContent></Select><Select value={sort} onValueChange={setSort}><SelectTrigger className="w-44"><SelectValue placeholder="Sort" /></SelectTrigger><SelectContent><SelectItem value="updated">Recently updated</SelectItem><SelectItem value="oldest">Oldest</SelectItem><SelectItem value="candidates">Most candidates</SelectItem></SelectContent></Select><div className="flex rounded-md border border-border-strong bg-surface p-0.5"><Button variant={view === 'list' ? 'default' : 'ghost'} size="icon-sm" aria-label="List view" onClick={() => setView('list')}><List className="h-4 w-4" aria-hidden="true" /></Button><Button variant={view === 'grid' ? 'default' : 'ghost'} size="icon-sm" aria-label="Grid view" onClick={() => setView('grid')}><Grid2X2 className="h-4 w-4" aria-hidden="true" /></Button></div></div></div>
        <div className="mt-4" aria-live="polite">{loading ? <div className="space-y-3"><JobCardSkeleton /><JobCardSkeleton /><JobCardSkeleton /></div> : sortedJobs.length === 0 ? <div className="flex min-h-[280px] flex-col items-center justify-center rounded-lg border border-dashed border-border-strong bg-surface px-6 text-center"><CircleAlert className="h-6 w-6 text-muted" aria-hidden="true" /><h2 className="mt-3 text-h2 text-heading">No jobs match your search</h2><p className="mt-2 text-body text-muted">Try a different search or clear the filters.</p><Button variant="secondary" className="mt-5" onClick={() => { setSearch(''); setStatus('all'); }}>Clear filters</Button></div> : <div className={cn(view === 'grid' ? 'grid gap-3 md:grid-cols-2' : 'space-y-3')}>{shownJobs.map((job) => <JobCard key={job.id} job={job} archived={tab === 'archived'} view={view} onAction={handleAction} />)}</div>}</div>
