...
> **Status: implemented.** Geist Sans/Mono, the full gray + accent scales, and the semantic token
> layer are live across the app. See §24 for exactly how the spec maps onto this codebase, which
> parts are done, and what is deliberately outstanding.
...
> **Sourcing note.** Backgrounds, the gray scale, gray-alpha, and blue are taken directly from
> Vercel's published Geist token file. The red / amber / green / purple steps above and the dark
> theme in §3.1 are the standard Geist values but were not read from that file. Confirm them at
> `vercel.com/geist/colors` — right-click any swatch to copy the raw value — before locking the theme.
...
- **Set:** Geist Icons or Lucide — both are 1.5px outline families sized correctly for Geist Sans.
  Do not mix families.
...
How this spec is wired into the codebase. Read this before changing any visual token.

### 24.1 Where the tokens live

| Concern | File |
| --- | --- |
| All raw scales + semantic aliases, light and dark | `app/globals.css` (`@layer base`) |
| Tailwind colour/type/font mapping | `tailwind.config.ts` |
| Font loading (`GeistSans`, `GeistMono`) | `app/layout.tsx` |

**The gray, blue, red, amber, green and purple scales are exposed as Tailwind colours**, so
`bg-gray-100`, `text-gray-900`, `border-gray-400` etc. all work. These intentionally shadow
Tailwind's built-in palettes — the built-ins are not used anywhere in this codebase.

### 24.2 The semantic layer is the migration mechanism

Rather than rewrite ~140 component files, the existing semantic variables were **repointed** at
Geist values. Every component keeps its class names; only the resolved colour changed.

| Existing class | Was | Now resolves to |
| --- | --- | --- |
| `bg-primary` | `#5B4FE9` indigo | `gray-1000` — the ink |
| `text-primary` | indigo | `gray-1000` `#171717` |
| `text-heading` | `#1F242E` | `gray-1000` `#171717` |
| `text-bodyText` | `#4B5563` | `gray-1000` — Tier 1, per §4 |
| `text-muted` | `#6B7280` | `gray-900` `#4d4d4d` — Tier 2 |
| `border-border` | `#E5E7EB` | `gray-400` `#eaeaea` |
| `bg-background` | `#F6F5FF` lilac | `background-100` `#ffffff` |
| `bg-active-menu-bg` | `#EEF2FF` indigo tint | `gray-100` `#f2f2f2` |
| `bg-hero-gradient` | indigo gradient | tonal `background-200 → gray-100` |

**Consequence:** `text-bodyText` and `text-heading` are now the same value. That is correct per §4 —
body copy is Tier 1. Use `text-muted` for the Tier 2 sub-text tier, and nothing for Tier 3 except
placeholders and disabled states.

### 24.3 Radius

`--radius-sm/md/lg` all resolve to **6px**; `--radius-xl` is 8px for modals. This means the existing
`rounded-md`, `rounded-lg` and `rounded-xl` classes scattered through the app collapse onto the
correct values without touching each call site. `rounded-full` remains for pills and avatars only.

### 24.4 Utilities added

| Class | Purpose |
| --- | --- |
| `.tabular` | `font-variant-numeric: tabular-nums` for numeric columns (§2) |
| `.mono` | Geist Mono for identifiers, scores, timestamps |
| `.eyebrow` | The mono uppercase eyebrow — 12px, `+0.4px` tracking, `gray-900` (§5) |
| `.ring-hairline` | `box-shadow: 0 0 0 1px` instead of a CSS border (§7) |

### 24.5 Disabled buttons

§8 requires a `gray-100` fill with `gray-700` text. Roughly 15 call sites used
`bg-primary … disabled:opacity-50`, which under the new palette rendered as a *dimmed near-black
slab* — still reading as the page's main action. Rather than edit every call site, a single base-layer
rule in `globals.css` overrides `button:disabled.bg-primary`. If you add a new primary button, you get
the correct disabled state for free.

### 24.6 Charts

`app/(dashboard)/reports/page.tsx` follows §14: the gray ramp with **one** blue focal series.
`Invited` is context (`#171717` light / `#ededed` dark); `Responded` carries the argument
(`#006bff` / `#0072f5`). Chart colours must be concrete hex — Recharts cannot resolve CSS custom
properties inside the SVG — so they are declared in the `SERIES` constant and switched on the active
theme, not read from tokens.

### 24.7 Deliberately outstanding

These are known gaps, not oversights. Each needs a design decision or a content pass, so none were
silently guessed at:

1. **Uppercase sidebar group labels.** §10 prefers a `gray-400` hairline over eyebrow labels.
   `WORKSPACE / PRODUCT / PERSONAL / ADVANCED` are still uppercase eyebrows in
   `components/settings/settings-sub-nav.tsx` and `components/sidebar/sidebar.tsx`. Swapping them for
   hairlines removes the group *names*, which is a product decision, not a styling one.
2. **Numeric table columns are not yet Geist Mono.** §12 requires right-aligned mono with tabular
   figures, headers included. Alignment is correct; the mono swap is not applied.
3. **The AI marker (§16)** — mono `AI` label + sparkle icon — is not yet applied to AI-generated
   surfaces. `--accent-ai` currently points at `purple-700` so nothing renders coral, but the carrier
   itself still needs to be introduced where AI output actually appears.
4. **Colour-picker swatches** in `settings/career` and the job-branding wizard remain raw hex. These
   are user-facing colour choices for *candidate-facing* pages, not admin chrome, so they are
   correctly outside the achromatic system.
5. **`--accent-ai` is retained as a token** for backwards compatibility with existing class names.
   Per §16 it should not be used as a fill; prefer the mono label + sparkle.

### 24.8 Verifying a change

```bash
npx tsc --noEmit && npm run build
```

Then check both themes. Dark mode is toggled by the `.dark` class on `<html>` — every token has a
dark value, so a component that uses only semantic classes needs no dark-specific code.
